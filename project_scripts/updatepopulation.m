function [ sampleupdate_alpha, sampleupdate_d, sampleupdate_beta ]=updatepopulation(N,samplenow_alpha,samplenow_d,samplenow_beta,num_pop_SLL,pop)
...
%   sampleupdate_beta=zeros(pop,N);
  ...
  for q=1:10
    ...
    % magnitude update
    sampleupdate_beta(q,:)=samplenow_beta(q,:)+rb(1)*(best_beta-abs(samplenow_beta(q,:)))-rb(2)*(worst_beta-abs(samplenow_beta(q,:)));
  end
